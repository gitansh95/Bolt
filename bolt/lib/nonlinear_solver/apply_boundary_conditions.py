...
def apply_horizontal_internal_mirror_bcs_f(self, q2_index,
                                           mirror_start, mirror_end
                                          ):
    mirror_indices = ((self.q1_center >= mirror_start) & \
                            (self.q1_center <= mirror_end))

    #print ("Mirror indices shape 1 : ", mirror_indices.shape)
    ...
    N_g    = self.N_ghost
    center = q2_index#int(self.N_q2/2 + N_g) 

    #print ('f shape', self.f[:, :, center-2*N_g:center-N_g].shape)
    #print ('mirror indices shape', mirror_indices[:, :, center-N_g:center].shape)
    ...
    self.f[:, :, center-N_g:center] = \
            mirror_indices[:, :, center-N_g:center]*af.flip(self.f[:, :, center-2*N_g:center-N_g], 2) + \
    ...
    self.f[:, :, center-N_g:center] = \
        mirror_indices[:, :, center-N_g:center] * \
        self._convert_to_q_expanded(af.flip(self._convert_to_p_expanded(self.f), 1))[:, :, center-N_g:center] + \
        mirror_indices_inv[:, :, center-N_g:center]*self.f[:, :, center-N_g:center]
    ...
    self.f[:, :, center:center+N_g] = \
            mirror_indices[:, :,  center:center+N_g]*af.flip(self.f[:, :, center+N_g:center+2*N_g], 2) + \
    ...
    # contains the variation in p2
    self.f[:, :, center:center+N_g] = \
        mirror_indices[:, :, center:center+N_g] * \
        self._convert_to_q_expanded(af.flip(self._convert_to_p_expanded(self.f), 1))[:, :, center:center+N_g] + \
        mirror_indices_inv[:, :, center:center+N_g]*self.f[:, :, center:center+N_g]
...
def apply_bcs_f(self):
    ...
    #        apply_vertical_internal_mirror_bcs_f(self,
    #                self.physical_system.params.mirror_1_index, 7.0, 8.0)

    if ((i_q2_start < self.physical_system.params.horizontal_mirror_0_index) and \
        (i_q2_end > self.physical_system.params.horizontal_mirror_0_index)):
