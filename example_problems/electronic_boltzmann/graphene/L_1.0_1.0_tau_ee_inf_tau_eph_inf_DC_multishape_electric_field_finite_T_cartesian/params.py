...
right_dirichlet_boundary_index  = 145  # Default value is N_q1-1  
...
# Time parameters:
dt      = 0.025/16 # ps #Data before 137.5 ps is using dt = 0.025/16
t_final = 40.     # ps
...
def electric_field(q1, q2, p1, p2, p3):
    ...
    if (rank == 0):
        ...
        E_x = np.loadtxt("E_x_1.txt")[:, :int(N_q2_local)]
        E_y = np.loadtxt("E_y_1.txt")[:, :int(N_q2_local)]
        print("Rank 0, E_x shape : ", E_x.shape)

    elif (rank == 1):

        # Calculate the derivatives
        E_x = np.loadtxt("E_x_2.txt")[:, -int(N_q2_local):]
        E_y = np.loadtxt("E_y_2.txt")[:, -int(N_q2_local):]
        print("Rank 1, E_x shape : ", E_x.shape)
