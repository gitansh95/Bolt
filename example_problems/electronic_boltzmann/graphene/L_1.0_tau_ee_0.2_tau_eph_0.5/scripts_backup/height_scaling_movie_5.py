...
for number, index in enumerate(heights_2[:]):
    ...
    if (N_q2 <= int(N_q2_full/2)):
        ...
        ax1.axvline(q2_start,
                ymin=0,#q1_temp[int(N_q1_temp/2)-1]/q1_end_temp,
                ymax=1,#q1_temp[-1]/q1_end_temp,
    ...
    # If plot of data requires 2 rows
        ...
        ax3.axhline(-q1_temp[int(N_q1_temp/2)],
                xmin=q2_start/q2_end_temp,
    ...
    im2 = ax3.contourf(q1_temp, q2_temp, density_full_array, 200,
                norm=colors.SymLogNorm(linthresh=density_full_array.max()/20,
                    vmin=density_min, vmax=density_max), cmap='bwr',
    ...
    ax3.streamplot(q1_temp, q2_temp,
                ...
                j_y_2_full_array,
                density=8, color='black',
                linewidth=1.2, arrowsize=1.4, transform = rot2 + base2
                  )
    ...
    pl.savefig('images/dump_tau_mr_inf_%06d.png'%number)
    pl.clf()
