...
tau_1 = np.arange(0.01, 0.191, 0.01)
tau_2 = np.arange(0.20, 0.501, 0.01)
tau_3 = np.arange(0.6, 10.01, 0.1)
tau_4 = np.arange(11.0, 99.01, 1.0)

tau = tau_1
tau = np.append(tau, tau_2)
tau = np.append(tau, tau_3)
tau = np.append(tau, tau_4)
...
for index in tau:
    print ('Index : ', index)
    filepath = \
    '/home/mchandra/gitansh/gitansh_bolt/example_problems/electronic_boltzmann/L_1.0_2.5_tau_ee_%.2f_tau_eph_100.00_DC/dumps'%index
...
pl.plot(tau, mode_zero_array, '-o', label = '$\mathrm{k = 0}$')
pl.plot(tau, mode_one_array, '-o', label = '$\mathrm{k = 1}$')
pl.plot(tau, mode_others_array,'-o', label='$\mathrm{k > 1}$')

pl.xlabel('$\mathrm{\\tau_{mc}\ (ps)}$')
...
pl.savefig('images/mode_vs_tau.png')
pl.clf()
