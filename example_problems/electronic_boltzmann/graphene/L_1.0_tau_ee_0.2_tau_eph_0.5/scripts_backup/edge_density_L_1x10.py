...
tau_1 = np.array([0.1, 0.5, 0.6, 0.7])
tau = np.arange(1.0, 9.01, 1.0)

tau = np.append(tau_1, tau)
...
for index in tau:
    ...
    filepath = \
	    "dumps_L_1.0_20.0_tau_eph_%.1f"%(index) #TODO
    ...
    np.savez_compressed("edge_data/edge_L_1.0_10.0_tau_ee_inf_tau_eph_%.2f.txt"%(index), left=voltage_left, right=voltage_right)
