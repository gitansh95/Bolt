...
heights = np.arange(20.00, 20.01, 0.1)
...
for l in heights:
    ...
    for file_number, dump_file in enumerate(moment_files[:-1]):
        ...
        j_x_2 = lagrange_multipliers[:, :, 3]
        j_y_2 = lagrange_multipliers[:, :, 4]
    ...
    np.savez_compressed("edge_data/edge_L_1.0_%.2f_tau_ee_inf_tau_eph_10.0.txt"%(l/2), left=voltage_left, right=voltage_right)
