...
for index in np.arange(start_index, end_index, step):
    ...
    source_filepath = \
        current_filepath + '/dumps_wire_tau_%.2f'%index

    filepath = \
    '/home/mchandra/cci_data/dumps_wire'
    ...
    os.system("cp " + moment + " " + filepath + \
            "/moments_L_1.0_0.25_tau_ee_inf_tau_eph_%.2f.h5"%(index))
    os.system("cp " + lagrange + " " + filepath + \
            "/lagrange_multipliers_L_1.0_0.25_tau_ee_inf_tau_eph_%.2f.h5"%(index))
    os.system("cp " + moment_info + " " + filepath +
            "/moments_L_1.0_0.25_tau_ee_inf_tau_eph_%.2f.h5.info"%(index))
    os.system("cp " + lagrange_info + " " + filepath +
            "/lagrange_multipliers_L_1.0_0.25_tau_ee_inf_tau_eph_%.2f.h5.info"%(index))
