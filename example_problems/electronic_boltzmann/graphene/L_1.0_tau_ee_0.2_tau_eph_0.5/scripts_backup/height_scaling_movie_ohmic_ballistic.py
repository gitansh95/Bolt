...
for number, index in enumerate(heights_2[115:116]):
    print ('Index : ', index)
    ...
    filepath_ohmic = \
        '/home/mchandra/cci_data/dumpsi_ohmic_L_1.0_%.2f'%index
    ...
    moment_files 		  = np.sort(glob.glob(filepath_ohmic+'/moment*.h5'))
    lagrange_multiplier_files = \
            np.sort(glob.glob(filepath_ohmic+'/lagrange_multipliers*.h5'))
    ...
    lagrange_multipliers = io.readBinaryFile(lagrange_multipliers_file)
    lagrange_multipliers = lagrange_multipliers[0].reshape(N_q2, N_q1, 7)
    j_x_2_ohmic = lagrange_multipliers[:, :, 5]
    j_y_2_ohmic = lagrange_multipliers[:, :, 6]

    delta_n_ohmic = density_ohmic - np.mean(density_ohmic)
    print ('Ohmic density min/max : ', np.min(delta_n_ohmic), np.max(delta_n_ohmic))
    ...
    # If plot of data fits in the top row
    if (N_q2 <= int(N_q2_temp)):
        density_full_array_2[:N_q2, :] = delta_n_ohmic
        j_x_2_full_array_2[:N_q2, :]   = j_x_2_ohmic
        j_y_2_full_array_2[:N_q2, :]   = j_y_2_ohmic
    
        ax3.axvline(q2_start,
                color='k', lw=2) #Left spine
    ...
    ax2.plot(heights[number+115]-0.25,
            resistance_local_list_tau_mr_10_vel_1em3[number+115]/norm,
            color='red', marker='+', mew = 5,  markersize=15)

    ax2.set_xlim(xmin=0.0, xmax=9.75)
    ax2.set_ylim(ymax=1.70)

    ax2.text(8.85, 0.05, "$ %.2f$"%(heights[number+115] - 0.25))
    ...
    pl.savefig('images/dump_comparision_%06d.png'%number)
    pl.clf()
