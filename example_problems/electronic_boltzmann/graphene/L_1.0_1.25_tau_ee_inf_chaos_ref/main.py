...
while t0 < t_final:
    ...
    density = nls.compute_moments('density')
    density_np = density.to_ndarray()

    left_edge_density.append(density_np[0, N_g, N_g:-N_g])
    right_edge_density.append(density_np[0, -N_g-1, N_g:-N_g])

    #print ("1 : ", left_edge_density.size)
    ...
    #else:
    #    dump_steps = params.dump_steps
    if (time_step%params.dump_dist_after==0):
    ...
    if (time_step%dump_steps==0):
        ...
        #print ("2 : ", left_edge_density.shape)

        if (params.rank==0):
            np.savetxt("dump_time_array.txt", dump_time_array)
            np.savez_compressed("../data_3/voltages_L_1.0_2.5_tau_ee_inf_tau_eph_%.6f.txt"%(tau), left=voltage_left, right=voltage_right)
