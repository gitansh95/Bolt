...
for file_number, dump_file in enumerate(moment_files[:]):
    ...
    density = j_x#density - density_bg
    density_max = 3.#np.max(np.abs(density))
    density_min = -density_max#np.min(density)
    #density = j_y#density - density_bg
    #density_min = np.min(density)
    #density_max = np.max(density)

    injection_indices  = (y[0, :] > 0. ) & (y[0, :] < 1.25)
    J_in = np.abs(np.mean(j_x[injection_indices, 0]))
    
    J = np.sqrt(j_x**2 + j_y**2)
    j_x_m = np.ma.masked_where(J < 12e-3, j_x)
    j_y_m = np.ma.masked_where(J < 12e-3, j_y)
...
#    plot_grid(x[::1, ::1], y[::1, ::1], alpha=0.5)
    ...
    m = pl.contourf(x, y,
        j_x.T, 200,
        norm=colors.SymLogNorm(linthresh=np.max(np.abs(j_x))/20, vmin=density_min, vmax=density_max),
    ...
    density = j_y#density - density_bg
    density_max = 0.5#np.max(np.abs(density))
    density_min = -density_max#np.min(density)
...
#    plot_grid(x[::1, ::1], y[::1, ::1], alpha=0.5)
    ...
    m = pl.contourf(x, y,
        j_y.T, 200,
        norm=colors.SymLogNorm(linthresh=np.max(np.abs(j_y))/20, vmin=density_min, vmax=density_max),
        cmap='bwr', transform = rot + base)
    m = pl.cm.ScalarMappable(cmap='bwr')
    m.set_array(j_y.T)
    m.set_clim(density_min, density_max)
