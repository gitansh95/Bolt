...
for file_number, dump_file in enumerate(moment_files[:]):
    ...
    injection_indices  = (y[0, :] > 0.5 ) & (y[0, :] < 0.7)
    J_in = np.mean(j_x[injection_indices, :])
    
    J = np.sqrt(j_x**2 + j_y**2)
    j_x_m = np.ma.masked_where(J < 1e-2*J_in, j_x)
    j_y_m = np.ma.masked_where(J < 1e-2*J_in, j_y)
...
#    plot_grid(x[::1, ::1], y[::1, ::1], alpha=0.5)
    ...
    #pl.contourf(x, y, mask[2:-2, 2:-2], 100, cmap = 'gray', alpha = 0.5)
    #pl.xlim([0., 1.])
    #pl.ylim([0, 1.])
